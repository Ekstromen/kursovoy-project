		...
		for (x = 0; x < N; x++) //построчная проверка на наличие цифры в ячейке массива
		{
			if (playing_field[y][x] != '*' && playing_field[y][x] != '#' && playing_field[y][x] != '1')
			{
				for (int y1 = y; y1 < N; y1 += 1) 
				{
					...
					{
						if (playing_field[y - 1][x] == '*' || (playing_field[y - 1][x] == playing_field[y][x])) //переход на ячейку сверху для проверки наличия вней #
						{
							count += 1;
							y1 -= 1;
						}
						if (playing_field[y + 1][x] == '*' || (playing_field[y + 1][x] == playing_field[y][x]))	//переход на ячейку снизу для проверки наличия вней #
						{
							count += 1;
							y1 += 1;
						}
						if (playing_field[y][x - 1] == '*' || (playing_field[y][x - 1] == playing_field[y][x]))	//переход на ячейку слева для проверки наличия вней #
						{
							count += 1;
							x1 -= 1;
						}
						if (playing_field[y][x + 1] == '*' || (playing_field[y][x + 1] == playing_field[y][x]))	//переход на ячейку справа для проверки наличия вней #
						{
							count += 1;
							x1 += 1;
						}
